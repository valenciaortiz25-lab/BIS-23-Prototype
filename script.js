...
function calculateTotals() {
 ...
 // ===== jQuery VERSION (UPDATED TO MATCH) =====
...
let subtotal = 0;
    ...
    // selectors + value retrieval
    let taxRate = parseFloat($("#taxRate").val()) || 0;
    let extraCosts = parseFloat($("#extraCosts").val()) || 0;
    ...
    // DOM updates (jQuery methods)
    $("#subtotal").text("$" + subtotal.toFixed(2));
    $("#total").text("$" + total.toFixed(2));

    // CSS change example (extra requirement boost)
 ...
 */
$(document).on("input", ".amount, #taxRate, #extraCosts", calculateTotals);